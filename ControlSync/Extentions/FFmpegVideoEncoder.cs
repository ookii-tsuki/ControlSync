...
{
    public sealed unsafe class FFmpegVideoEncoder1 : IVideoEncoder, IDisposable
    {
        ...
        public FFmpegVideoEncoder1(Dictionary<string, string>? encoderOptions = null, AVHWDeviceType HWDeviceType = AVHWDeviceType.AV_HWDEVICE_TYPE_NONE)
        {
        ...
        private void InitialiseEncoder(AVCodecID codecID, int width, int height, int fps)
        {
            if (!_isEncoderInitialised)
            {
                ...
                {
                    //_encoderContext->profile = ffmpeg.FF_PROFILE_H264_CONSTRAINED_BASELINE;
                    ...
                    ffmpeg.av_opt_set(_encoderContext->priv_data, "preset", "ultrafast", 0);
                    ffmpeg.av_opt_set(_encoderContext->priv_data, "tune", "zerolatency", 0);
